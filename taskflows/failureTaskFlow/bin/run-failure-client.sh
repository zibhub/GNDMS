...
cp="$GNDMS_SOURCE/taskflows/failureTaskFlow/client/failure-client/production:$cp"

props=( )

exec java -cp $cp ${props[@]} de.zib.gndms.taskflows.failure.client.tools.FailureExample ${args[@]} "$@"
